...
def AQH_Device( 
    ...
    Lattice =  Device << AQH_Lattice(Nx=Nx, Ny=Ny, Gap=Gap, BendRadius = BendRadius, LengthSR = LengthSR, eta = eta, WgWidth = WgWidth, Euler = Euler, Layer = Layer)
    LatticeLengthX = (Nx-1) * 2*(Gap + WgWidth + 2*BendRadius + LengthSR/2 + LengthLR/2) + 2*(LengthSR/2 + BendRadius)
    LatticeLengthY = (Ny) * 2*(Gap + WgWidth + 2*BendRadius + LengthSR/2 + LengthLR/2) + 2*(LengthLR/2 + BendRadius)
    ...
    I_TotLengthY = LatticeLengthY + 1.5*BendRadiusIO
    InLengthY    = DeltaY + LengthSR/2 + 3*BendRadius + Gap + WgWidth + LengthLR

    I = Device <<IO_2D_1(TotLengthX = TotLengthX,
                         TotLengthY = I_TotLengthY,
                         InLengthX  = InLengthX,
                         InLengthY  = InLengthY,
                         MidLengthX = 0.5*BendRadius,
